...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg font-bold text-white">Class Attendance</h1>
        </div>
    </div>
                    ...
                    <div class="flex flex-shrink-0" style="width: calc(100% + 72px); min-width: calc(100% + 72px); scroll-snap-align: start;">
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-800/40 flex-shrink-0" style="width: calc(100% - 72px); min-width: calc(100% - 72px);">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @endif
                                <form action="{{ route('class.attendance.toggle', [$class->id, $booking->id]) }}" method="POST" class="flex-shrink-0">
                                    @csrf
                        ...
                        <form action="{{ route('class.attendance.booking.remove', [$class->id, $booking->id]) }}" method="POST" class="flex-shrink-0 w-[72px] flex items-center justify-center bg-red-500/90 rounded-r-xl" onclick="event.stopPropagation()">
                            @csrf
                            @method('DELETE')
            ...
            <div class="attendee-item rounded-xl overflow-hidden border border-slate-700/30 border-l-4 border-l-amber-500/50" data-name="{{ strtolower($trial->name) }}">
                <div class="overflow-x-auto scrollbar-hide" style="-webkit-overflow-scrolling: touch; scroll-snap-type: x mandatory;">
                    <div class="flex flex-shrink-0" style="width: calc(100% + 72px); min-width: calc(100% + 72px); scroll-snap-align: start;">
                        ...
                        <form action="{{ route('class.attendance.trial.remove', [$class->id, $trial->id]) }}" method="POST" class="flex-shrink-0 w-[72px] flex items-center justify-center bg-red-500/90 rounded-r-xl" onclick="event.stopPropagation()">
                            @csrf
                            @method('DELETE')
            ...
            <div class="space-y-2" id="walkInList">
                @foreach($availableMembers as $member)
                    <div class="walkin-item flex items-center gap-3 p-3 rounded-xl bg-slate-800/20 border border-slate-700/20"
                        ...
                        <form action="{{ route('class.attendance.walkin', $class->id) }}" method="POST" class="flex-shrink-0">
                            @csrf
